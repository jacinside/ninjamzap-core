...
public:
  ...
  void Encode(float *in, int inlen, int advance=1, int spacing=1) // length in sample (PAIRS)
  {
    ...
    // Check for valid input when expected
    if (in == NULL && inlen > 0) {
      m_err = -1;
      return;
    }
    ...
    if (inlen == 0)
    {
      if (vorbis_analysis_wrote(&vd, 0) != 0) {
        m_err = -2;
        return;
      }
    }
    else
    {
      ...
      if (!vd.pcm && inlen > 0) {
          fprintf(stderr, "🚫 [VorbisEncoder] Encoder not initialized (vd.pcm is null)\n");
          return;
      }
      ...
      if (vorbis_analysis_wrote(&vd, i) != 0) {
        m_err = -5;
        return;
      }
    }
    ...
    while((blockout_result = vorbis_analysis_blockout(&vd, &vb)) == 1)
    {
      ...
      while(vorbis_bitrate_flushpacket(&vd, &op))
      {
        ...
        while (!eos)
        {
          ...
          // Ensure we have enough space in the output queue
          if (!outqueue.Add(og.header, og.header_len) || !outqueue.Add(og.body, og.body_len)) {
            m_err = -9; // Queue allocation failure
            return;
          }
    ...
    // Check for unexpected blockout error
    if (blockout_result < 0) {
      m_err = -10;
      return;
    }
  }
