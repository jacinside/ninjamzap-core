...
void NinjamClient::disconnect() {
  ...
  stopConnectionThread = true;
  std::this_thread::sleep_for(std::chrono::milliseconds(100)); // ⚠️ solo parche
...
void NinjamClient::audiostreamOnSamples(float **inbuf, int innch,
                                        ...
                                        int srate) {
  L_(ldebug) << "[NinjamClient] Entering NinjamClient::audiostreamOnSamples with frame rate: "
              << srate;
  if (!connected) {
...
//  for (int i = 0; i < len * outnch; ++i) {
//      if (std::fabs(outbuf[i]) > 0.001f) {
//          hasOutput = true;
//          break;
//      }
//  }
//  if (hasOutput) {
//      std::cout << "🔊 Audio remoto mezclado OK en outBuffer!" << std::endl;
//  } else {
//      std::cout << "🛑 outBuffer está en silencio" << std::endl;
//  }
}
...
std::vector<AbNinjam::Common::RemoteUser> NinjamClient::getRemoteUsers() {
  ...
  for (int useridx = 0; useridx < numUsers; useridx++) {
    ...
    for (int channelidx = 0; channelidx < MAX_USER_CHANNELS; channelidx++) {
      ...
      int outIndex = -1;
      char *result =
      njClient->GetUserChannelState(useridx, channelidx, nullptr, &vol, nullptr, nullptr, nullptr, &outIndex, &flags);
      if (result != 0) {
        // Detect stereo from the actual decoded Vorbis stream channel count
        // Returns 0 if decoder not yet active, 1 for mono, 2 for stereo
        int decodedNch = njClient->GetUserChannelDecodedNch(useridx, channelidx);
        bool isStereo = (decodedNch >= 2);
...
//int useridx, int channelidx,bool setsub, bool sub, bool setvol, float vol, bool setpan, float pan, bool setmute, bool mute, bool setsolo, bool solo, bool setoutch, int outchannel)
void NinjamClient::setUserChannelState(int userId, int channelId,bool setsub, bool sub, bool setvol, float vol, bool setpan, float pan, bool setmute, bool mute, bool setsolo, bool solo, bool setoutch, int outchannel) {
  L_(ltrace) << "[NinjamClient] Entering NinjamClient::setUserChannelState";
  L_(ltrace) << "[NinjamClient] userId: " << userId;
  L_(ltrace) << "[NinjamClient] channelId: " << channelId;
  L_(ltrace) << "[NinjamClient] volume: " << vol;
  if (njClient) {
    njClient->SetUserChannelState(userId, channelId, setsub, sub, setvol, vol,
                                  setpan, pan, setmute, mute, setsolo, solo, setoutch, outchannel);
  }
}
